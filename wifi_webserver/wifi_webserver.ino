...
void setup() {
  ...
  // v1
  server.on("/changeTarget", handle_changeTarget);
...
void loop() {
  while(1) {
    delay(10000);
    server.handleClient();
    Serial.println("test-remove later");
  }
  ///*
  // v0
  if(LED1status)
...
void handle_OnConnect() {
  ...
  server.send(200, "text/html", SendHTML(LED1status,LED2status)); //v0
  //server.send(200, "text/html", SendHTML()); //v1
}
...
String SendHTML(uint8_t led1stat,uint8_t led2stat){ //v0
//String SendHTML(){ //v1
